...
class GPT:
    ...
    def call_qwen(self, content, args={}, showkeys=False):
        print('-----Patient-----')
        from openai import OpenAI

        client = OpenAI(
            # 若没有配置环境变量，请用百炼API Key将下行替换为：api_key="sk-xxx",
            api_key="sk-f3ff09af79634cafa4a32a498888ebf6",
        ...
        # response = json.loads(completion.choices[0].message.content.decode("utf-8"))
        print(response)
    ...
    def retry_call(self, content, args={}):
        ...
        return self.call_qwen(content, args)
...
def test(args):
    ...
    with torch.no_grad():
        ...
        dataloader_iterator = tqdm(val_dataloader,
                                   total=len(val_dataloader)) if accelerator.is_main_process else val_dataloader
        ...
        for batch in dataloader_iterator:
            ...
            for da in data:
                ...
                # all_sym_info = ', '.join(
                #     [k if v else f'没有"{k}"' for k, v in da['explicit_symptoms']] + [k if v else f'没有"{k}"' for k, v
                #                                                                       in da['implicit_symptoms']])

                # for english
                exp_request = ', '.join([k if v else f'No "{k}"' for k, v in da['explicit_symptoms']])
                all_sym_info = ', '.join(
                    [k if v else f'No "{k}"' for k, v in da['explicit_symptoms']] + [k if v else f'No "{k}"' for k, v in
                                                                                     da['implicit_symptoms']])

                query = exp_request
                chatbot.history = []
                print("explicit symptom")
                print(query)
                # 记录问诊流程
                single_dia = []
                single_dia.append({"explicit symptom": query})
                for jj in range(max_inquiry):
                    curr_dia = []
                    print(f'-----round:{jj + 1}-----')
                    # start conversation
                    for ii in range(try_time):
                        cur_generate, newhistory, confidence_distribution = chatbot.inference(query, history,
                                                                                              da['candidate_diseases'])
                        print('-----Doctor-----')
                        print(cur_generate)

                        curr_dia.append({"doctor": cur_generate,"confidence_distribution": confidence_distribution})
                        if len(confidence_distribution) > 0:
                            break

                    if 'Diagnosis:' in cur_generate or '做出诊断:' in cur_generate:
                        curr_dia.append({"finish": "yes"})
                        single_dia.append({"round": jj + 1, "content": curr_dia})
                        break

                    if jj == max_inquiry-1:
                        # Last turn
                        curr_dia.append({"finish": "no"})
                        single_dia.append({"round": jj + 1, "content": curr_dia})
                        break
                    ...
                    if jj != max_inquiry - 1:
                        ...
                        query = gpt.call_qwen(gpt4query)
                        curr_dia.append({"instruct": gpt4query, "patient": query})
                        history = newhistory
                        if isdebug:
                            isdebug = False
                            print(f'gpt4 query {gpt4query}\nresponse{query}\n', flush=True)
                    single_dia.append({"round":jj+1,"content":curr_dia})
                dia_note.append(single_dia)
                # 写出
                with open(f'result/dia_note_{start_time}.json', 'w', encoding='utf-8') as f:
                    json.dump(dia_note, f, ensure_ascii=False, indent=4)
        ...
        if accelerator.is_main_process:
            task_name = os.path.split(args.model_path)[-1].split(".")[0] if 'tfmr' not in args.model_path else '-'.join(
                args.model_path.split('/')[-3:-1])
...
if __name__ == '__main__':
    ...
    # Model Args
    parser.add_argument('--data_path', default='./evaluation/Dxy_en.json', type=str)
    parser.add_argument('--model_path', default='/root/autodl-tmp/weights/FreedomIntelligence/DiagnosisGPT-6B',
                        type=str)
    parser.add_argument('--max_inquiry', default=3, type=int)
