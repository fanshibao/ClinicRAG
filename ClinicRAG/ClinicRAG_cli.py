...
class DiagnosisChatbot():
    ...
    def inference(self, query, history=[], candidate_diseases=None):
        ...
        # For diagnosis.
        if ('Enter the diagnostic process, analyzing patient symptoms:' in generated_text or
                'Analyzing patient symptoms:' in generated_text or
                '进入诊断流程，分析病人症状信息:' in generated_text or
                '总结病人症状信息:' in generated_text):

            if 'Enter the diagnostic process, analyzing patient symptoms:' in generated_text or \
                    'Analyzing patient symptoms:' in generated_text:
                is_en = True
            else:
                is_en = False
            ...
            if max_value > self.confidence_threshold:
                ...
                # 当诊断置信度不足时，调用 compute_symptom_similarity_set
                # 如果调用时没有传入 candidate_diseases，则从 candi 中提取疾病名称（假设候选疾病字典中包含 'disease_name' 字段）

                candidate_names = [cand.get('disease', 'Unknown') for cand in candi]


                special_symptoms = self.retriever_en.compute_symptom_similarity_set(candidate_names)
                ...
                if is_en:
                    inquiry_lines.append(
                        "\n\n\nInadequate diagnostic confidence. Please confirm if you experience the following symptoms associated with the candidate diseases:")
                else:
                    inquiry_lines.append("\n\n\n诊断置信度较低。请问您是否存在以下与候选疾病相关的特殊症状：")
                for disease, symptom in special_symptoms.items():
                    inquiry_lines.append(f" - {disease}: '{symptom}'")
                inquiry_text = "\n".join(inquiry_lines)

                cur_generate += inquiry_text
                generated_text = self.model_genrate(prompt, cur_generate)
                cur_generate += generated_text
