...
class encoder(nn.Module):
    ...
    def compute_symptom_similarity_set(self, candidate_diseases):
        """
        计算候选疾病之间的症状相似度，为每个候选疾病选出一个最特殊的症状
        特殊症状：该症状与其他候选疾病所有症状的平均相似度最低
        :param candidate_diseases: 候选疾病列表（通常为疾病名称或 ID，根据 self.disease2id 的映射）
        :return: 字典，映射每个候选疾病到其特殊症状
        """
        import numpy as np
        ...
        for disease in candidate_diseases:
            # 通过 self.disease2id 映射得到内部疾病 ID
            disease_id = self.disease2id[disease]
            # 假定 self.disease[disease_id] 为字典，包含 "symptoms" 字段
            candidate_symptoms[disease] = self.disease[disease_id]["common_symptom"]

        # 2. 计算每个候选疾病中各个症状的向量表示
        # 使用 encode 函数对每个症状进行编码，存入 symptom_embeddings 字典
        symptom_embeddings = {}
        for disease, syms in candidate_symptoms.items():
            symptom_embeddings[disease] = []
            # 如果 syms 是字符串，则按逗号拆分成列表
            if isinstance(syms, str):
                syms = [s.strip() for s in syms.split(',')]
            for sym in syms:
                emb = self.encode(sym)  # 输出 shape (1, hidden_size)
                emb = emb.flatten()  # 展平成一维向量
                symptom_embeddings[disease].append((sym, emb))

        # 3. 对于每个候选疾病，遍历其所有症状，计算与其他候选疾病中所有症状的平均余弦相似度
        disease_special_symptom = {}
        for disease, sym_emb_list in symptom_embeddings.items():
            best_sym = None
            best_avg_sim = float('inf')
            for sym, emb in sym_emb_list:
                total_sim = 0.0
                count = 0
                # 遍历其他候选疾病的症状
                for other_disease, other_sym_emb_list in symptom_embeddings.items():
                    if other_disease == disease:
                        continue
                    for other_sym, other_emb in other_sym_emb_list:
                        # 计算余弦相似度，避免除 0 情况，加上一个很小的常数 epsilon
                        cos_sim = np.dot(emb, other_emb) / ((np.linalg.norm(emb) * np.linalg.norm(other_emb)) + 1e-8)
                        total_sim += cos_sim
                        count += 1
                # 若 count > 0，则计算平均相似度，否则设为 0
                avg_sim = total_sim / count if count > 0 else 0.0
                # 选择平均相似度最低的症状作为特殊症状
                if avg_sim < best_avg_sim:
                    best_avg_sim = avg_sim
                    best_sym = sym
            disease_special_symptom[disease] = best_sym

        return disease_special_symptom
